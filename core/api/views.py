...
from core.models import Item, OrderItem, Order, Address, Payment, Coupon, Refund, UserProfile, Variation, ItemVariation ,SharedItem,Share, Sharelist, Category , Transport
...
def Sharedlist(request):
    if request.method == 'POST':
        ...
        if "share" in request.POST:
            ...
            try:
                if share.shared:
                    Share_list_list = Sharelist.objects.filter(share = share)
                    Share_list = Share_list_list[0]
                    ...
                    Share_list.save()
                    return render( request, 'core/share_now.html',{"share_link":1,"url_for_share":Share_list.url,"Shared_with":Share_list.shared_user})
                else:
                    share.shared = True
                    share.end_date = request.POST['enddate']
                    share.save()
                    ...
                    Share_list.save()
                    return render( request, 'core/share_now.html',{"share_link":1,"url_for_share":Share_list,"Shared_with":Share_list.shared_user})
            except Exception as e:
                print(str(e))
                return redirect('index')
    ...
    if request.user.is_authenticated:
        if request.method == 'GET':
            ...
            if request.user.userprofile.owner:
                ...
                csrf_token = django.middleware.csrf.get_token(request)
                query = Q(share__items__item__stock = True)
                ...
                for my_sharelist in queryset:
                    if my_sharelist.share.end_date >= datetime.datetime(year= int(date_today.year), month = int(date_today.month) , day = int(date_today.day) ,tzinfo=pytz.UTC):
                        for my_share_item in my_sharelist.share.items.all():
                            if my_share_item.item.stock and (my_share_item.item.category == category or my_share_item.item.category.parent == category or my_share_item.item.category.parent.parent == category) :
                                items.append(my_share_item.item)
                if len(items):
                    page = request.GET.get('page', 1)
                    paginator = Paginator(list(set(items)), 20)
